// import { useState, useEffect } from "react";

export const sheHtml = (year, source, id) => {
  //   const [data, setData] = useState();

  //   useEffect(() => {
  //     if (entNotDetail !== undefined) {
  //       setData(entNotDetail.data.entrance_notulen);
  //     }
  //   }, [entNotDetail]);
  console.log(year, source, id);
  return `
  <main style="box-sizing: border-box;">
  <div class="aheader"
  style="margin: 0; padding: 0; box-sizing: border-box; position: relative; display: flex; justify-content: center; text-align: center; margin-bottom: 1em; padding-bottom: 1em; border-width: 3px; border-bottom-style: solid; border-color: black; border-left-style: none; border-right-style: none; border-top-style: none;">
    <img
      src="data:image/jpeg;base64,"
      alt="alt text"
      style="margin: 0; padding: 0; box-sizing: border-box; width: 100px; height: 100px; position: absolute; left: 0;">
    <div class="header" style="margin: 0; padding: 0; box-sizing: border-box; position: relative;">
      <h2 style="margin: 0; padding: 0; box-sizing: border-box; font-weight: bold; font-size: 1.5em;">PT Bank Rakyat
        Indonesia (PERSERO), Tbk</h2>
      <h3 style="margin: 0; padding: 0; box-sizing: border-box; font-weight: bold; font-size: 1.17em;">AUDIT INTERN
        WILAYAH</h3>
      <div style="margin: 0; padding: 0; box-sizing: border-box; margin-top: 10px;">
        <div style="margin: 0; padding: 0; box-sizing: border-box; display: grid; grid-template-columns: 75px 1fr;">
          <h4
            style="margin: 0; padding: 0; box-sizing: border-box; font-weight: bold; font-size: 1em; justify-self: left;">
            Alamat</h4>
          <h4
            style="margin: 0; padding: 0; box-sizing: border-box; font-weight: bold; font-size: 1em; justify-self: left;">
            : Alamat...</h4>
        </div>
        <div style="margin: 0; padding: 0; box-sizing: border-box; display: grid; grid-template-columns: 75px 1fr;">
          <h4
            style="margin: 0; padding: 0; box-sizing: border-box; font-weight: bold; font-size: 1em; justify-self: left;">
            Fax</h4>
          <h4
            style="margin: 0; padding: 0; box-sizing: border-box; font-weight: bold; font-size: 1em; justify-self: left;">
            : Fax...</h4>
        </div>
        <div style="margin: 0; padding: 0; box-sizing: border-box; display: grid; grid-template-columns: 75px 1fr;">
          <h4
            style="margin: 0; padding: 0; box-sizing: border-box; font-weight: bold; font-size: 1em; justify-self: left;">
            No. Tlp</h4>
          <h4
            style="margin: 0; padding: 0; box-sizing: border-box; font-weight: bold; font-size: 1em; justify-self: left;">
            : Tlp...</h4>
        </div>
      </div>
    </div>
  </div>
  <div style="margin: 0; padding: 0; box-sizing: border-box; display: grid; grid-template-columns: 1fr 1fr;">
    <div style="margin: 0; padding: 0; box-sizing: border-box;">
      <div style="margin: 0; padding: 0; box-sizing: border-box; display: grid; grid-template-columns: 100px 1fr;">
        <p style="margin: 0; padding: 0; box-sizing: border-box;">Nomor</p>
        <p style="margin: 0; padding: 0; box-sizing: border-box;">: ...</p>
      </div>
      <div style="margin: 0; padding: 0; box-sizing: border-box; display: grid; grid-template-columns: 100px 1fr;">
        <p style="margin: 0; padding: 0; box-sizing: border-box;">Lampiran</p>
        <p style="margin: 0; padding: 0; box-sizing: border-box;">: ...</p>
      </div>
      <div style="margin: 0; padding: 0; box-sizing: border-box; display: grid; grid-template-columns: 100px 1fr;">
        <p style="margin: 0; padding: 0; box-sizing: border-box;">Perihal</p>
        <p style="margin: 0; padding: 0; box-sizing: border-box;">: Laporan Monitoring Realisasi Perbaikan Manajemen
          Kanca ... Periode Audit ... sd ...</p>
      </div>
    </div>
    <div style="margin: 0; padding: 0; box-sizing: border-box;">
      <p style="margin: 0; padding: 0; box-sizing: border-box;">Jakarta, {moment()
        .locale("id")
        .format("DD MMMM YYYY")}</p>
      <p style="margin: 0; padding: 0; box-sizing: border-box;">Kepada Yth :</p>
      <p style="margin: 0; padding: 0; box-sizing: border-box;">Pemimpin Wilayah Bank Rakyat Indonesia</p>
      <p style="margin: 0; padding: 0; box-sizing: border-box;">Di -</p>
      <p style="margin: 0; padding: 0; box-sizing: border-box;">…………………..</p>
    </div>
  </div>
  <div
    style="margin: 0; padding: 0; box-sizing: border-box; border: width 1px; border-bottom-style: solid; border-color: #737B7D; border-left-style: none; border-right-style: none; border-top-style: none; padding-left: 1rem; padding-bottom: 0.5rem; margin-top: 2rem; margin-bottom: 1rem;">
    <p style="margin: 0; padding: 0; box-sizing: border-box;">1. Surat Kanins No. ... Perihal Laporan Hasil Audit,
      tanggal ...</p>
    <p style="margin: 0; padding: 0; box-sizing: border-box;">2. Surat Kantor Cabang No. ..., tanggal ...</p>
  </div>
  <p style="margin: 0; padding: 0; box-sizing: border-box; text-indent: 30px; text-align: justify;">
    Menunjuk surat Kanins tersebut diatas perihal Laporan Hasil Audit Kanins pada Kanca X periode audit … sd …. serta
    Surat
    Kanca BRI X perihal Laporan Realisasi Perbaikan Manajemen, bersama ini kami sampaikan hasil evaluasi Realisasi
    Perbaikan
    Manajemen sampai dengan posisi tanggal …. sebagai berikut:
  </p>
  <table
    style="padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center; width: 100%; margin: 1rem 0;">
    <tr style="margin: 0; padding: 0; box-sizing: border-box;">
      <th
        style="margin: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center; background-color: #3C64B1; color: hsl(0, 0%, 100%); padding: 0.5rem 0;">
        <span style="margin: 0; padding: 0; box-sizing: border-box; color: hsl(0, 0%, 100%);">No</span></th>
      <th
        style="margin: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center; background-color: #3C64B1; color: hsl(0, 0%, 100%); padding: 0.5rem 0;">
        <span style="margin: 0; padding: 0; box-sizing: border-box; color: hsl(0, 0%, 100%);">Hasil Evaluasi
          RPM</span></th>
      <th
        style="margin: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center; background-color: #3C64B1; color: hsl(0, 0%, 100%); padding: 0.5rem 0;">
        <span style="margin: 0; padding: 0; box-sizing: border-box; color: hsl(0, 0%, 100%);">Σ Rekomendasi</span>
      </th>
      <th
        style="margin: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center; background-color: #3C64B1; color: hsl(0, 0%, 100%); padding: 0.5rem 0;">
        <span style="margin: 0; padding: 0; box-sizing: border-box; color: hsl(0, 0%, 100%);">%</span></th>
    </tr>
    <tr style="margin: 0; padding: 0; box-sizing: border-box;">
      <td
        style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center;">
        1.</td>
      <td
        style="margin: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: left; padding: 0.2rem 0.5rem;">
        Memadai</td>
      <td
        style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center;">
        {memadai}
      </td>
      <td
        style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center;">
        {percent_memadai}%
      </td>
    </tr>
    <tr style="margin: 0; padding: 0; box-sizing: border-box;">
      <td
        style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center;">
        2.</td>
      <td
        style="margin: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: left; padding: 0.2rem 0.5rem;">
        Tidak Memadai</td>
      <td
        style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center;">
        {tidak_memadai}
      </td>
      <td
        style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center;">
        {percent_tidak_memadai}%
      </td>
    </tr>
    <tr style="margin: 0; padding: 0; box-sizing: border-box;">
      <td
        style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center;">
        3.</td>
      <td
        style="margin: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: left; padding: 0.2rem 0.5rem;">
        Masih dalam pemantauan</td>
      <td
        style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center;">
        {dalam_pemantauan}
      </td>
      <td
        style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center;">
        {percent_dalam_pemantauan}%
      </td>
    </tr>
    <tr style="margin: 0; padding: 0; box-sizing: border-box;">
      <td colspan="2"
        style="margin: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: left; padding: 0.2rem 0.5rem; font-weight: bold;">
        TOTAL</td>
      <td
        style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center;">
        {total}
      </td>
      <td
        style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center;">
        {percent_total}%
      </td>
    </tr>
  </table>
  <p
    style="margin: 0; padding: 0; box-sizing: border-box; text-indent: 30px; text-align: justify; margin-bottom: 1rem;">
    Perlu kami sampaikan bahwa laporan monitoring ini hanya menilai upaya perbaikan kelemahan pengendalian intern yang
    telah
    dilakukan oleh Kanca X. Untuk itu kami harap Saudara tetap melakukan monitoring <strong
      style="margin: 0; padding: 0; box-sizing: border-box;">upaya mitigasi risiko</strong> atas kelemahan
    pengendalian intern tersebut sesuai rekomendasi yang telah disepakati.
  </p>
  <p
    style="margin: 0; padding: 0; box-sizing: border-box; text-indent: 30px; text-align: justify; margin-bottom: 1rem;">
    Selanjutnya untuk menilai <strong style="margin: 0; padding: 0; box-sizing: border-box;">efektifitas pengendalian
      intern tersebut</strong> kami akan melakukan pengecekan
    kembali pada audit berkala berikutnya.
  </p>
  <p
    style="margin: 0; padding: 0; box-sizing: border-box; text-indent: 30px; text-align: justify; margin-bottom: 1rem;">
    Demikian, kami sampaikan. Atas perhatian dan kerjasamanya kami sampaikan terima kasih.</p>
  <div style="padding: 0; box-sizing: border-box; text-align: center; margin: 1rem 0;">
    <p style="margin: 0; padding: 0; box-sizing: border-box; font-weight: bold; margin-bottom: 1rem;">REKAPITULASI
      MONITORING REALISASI PERBAIKAN MANAJEMEN</p>
    <table
      style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center; width: 100%;">
      <tr style="margin: 0; padding: 0; box-sizing: border-box;">
        <th
          style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center; background-color: #3C64B1; color: hsl(0, 0%, 100%);">
          <span style="margin: 0; padding: 0; box-sizing: border-box; color: hsl(0, 0%, 100%);">Unit Kerja
            Auditee</span></th>
        <th
          style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center; background-color: #3C64B1; color: hsl(0, 0%, 100%);">
          <span style="margin: 0; padding: 0; box-sizing: border-box; color: hsl(0, 0%, 100%);">No Exp</span></th>
        <th
          style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center; background-color: #3C64B1; color: hsl(0, 0%, 100%);">
          <span style="margin: 0; padding: 0; box-sizing: border-box; color: hsl(0, 0%, 100%);">Judul Temuan</span>
        </th>
        <th
          style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center; background-color: #3C64B1; color: hsl(0, 0%, 100%);">
          <span style="margin: 0; padding: 0; box-sizing: border-box; color: hsl(0, 0%, 100%);">Kategori Temuan</span>
        </th>
        <th
          style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center; background-color: #3C64B1; color: hsl(0, 0%, 100%);">
          <span style="margin: 0; padding: 0; box-sizing: border-box; color: hsl(0, 0%, 100%);">Rekomendasi</span>
        </th>
        <th
          style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center; background-color: #3C64B1; color: hsl(0, 0%, 100%);">
          <span style="margin: 0; padding: 0; box-sizing: border-box; color: hsl(0, 0%, 100%);">Batas Waktu RPM</span>
        </th>
        <th
          style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center; background-color: #3C64B1; color: hsl(0, 0%, 100%);">
          <span style="margin: 0; padding: 0; box-sizing: border-box; color: hsl(0, 0%, 100%);">Realisasi Perbaikan
            Manajemen</span></th>
        <th
          style="margin: 0; padding: 0; box-sizing: border-box; border: 1px solid black; border-collapse: collapse; text-align: center; background-color: #3C64B1; color: hsl(0, 0%, 100%);">
          <span style="margin: 0; padding: 0; box-sizing: border-box; color: hsl(0, 0%, 100%);">Status Tindak
            Lanjut</span></th>
      </tr>
    </table>
  </div>
</main>
`;
};
