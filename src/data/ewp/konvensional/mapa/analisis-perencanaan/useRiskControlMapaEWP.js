...
const useRiskControlMapaEWP = (params) => {
  const { risk_issue_id, search, limit } = params;

  const path = `${process.env.NEXT_PUBLIC_API_URL_EWP}/ewp/mapa/analisis_perencanaan/${risk_issue_id}/risk_control?search=${search}&limit=${limit}`;
