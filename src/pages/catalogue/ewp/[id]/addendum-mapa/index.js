...
const index = () => {
  ...
  const [addendumMapa, setAddendumMapa] = useState([]);
  const breadcrumbs = [
  ...
  useEffect(() => {
    if (addendum != undefined) {
      const mappingAddendumMapa = addendum.map((data, key) => {
        return {
          No: key + 1,
      ...
      setAddendumMapa(mappingAddendumMapa);
    }
  }, [addendum]);
