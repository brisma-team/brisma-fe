...
const index = () => {
  ...
  const { mapaEWP } = useMapaEWP("si", { id });
  ...
  useEffect(() => {
    setData({
      ...data,
      sumber_informasi: mapaEWP?.data ? mapaEWP?.data : "",
    });
  }, [mapaEWP]);
  ...
  const handlePost = async () => {
    try {
      return await usePostData(
        `${process.env.NEXT_PUBLIC_API_URL_EWP}/ewp/mapa/si`,
        data
      );
    } catch (e) {
      throw new Error(e);
