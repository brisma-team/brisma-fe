...
import { useDocumentEWP } from "@/data/ewp/konsulting/perencanaan/dokumen";
...
import {
  ...
  setWorkflowData,
  resetWorkflowData,
} from "@/slices/ewp/konsulting/analisa/documentMapaEWPKonsultingSlice";
...
import {
  useListDocument,
  useComment,
} from "@/data/ewp/konsulting/analisa/lingkup/dokumen";
...
const index = () => {
  ...
  const [listContent, setListContent] = useState([
    { idx: 1, total_comment: 0, title: "Analisa Data" },
    { idx: 2, total_comment: 0, title: "Resume" },
    { idx: 3, total_comment: 0, title: "Hasil Analisa" },
  ...
  const workflowData = useSelector(
    (state) => state.documentKkpaEWPKonsulting.workflowData
  );
  const validationErrorsWorkflow = useSelector(
    (state) => state.documentKkpaEWPKonsulting.validationErrorsWorkflow
  );

  const { projectDetail } = useProjectDetail({ id });
  const { listDocument } = useListDocument({ id: kkpa_id });
  const { documentEWP } = useDocumentEWP("kkpa", bab, { id: kkpa_id });
  const { workflowDetailEWP, workflowDetailEWPMutate } = useWorkflowDetailEWP(
    "kkpa",
    { id: kkpa_id }
  );
  const { comment, commentMutate } = useComment({
    id: kkpa_id,
  ...
  useEffect(() => {
    ...
    setBreadcrumbs([
      ...
      {
        name: `Dokumen`,
        path: `${pathNameLandingPage}/dokumen`,
      },
    ]);
  }, [projectDetail]);
  ...
  useEffect(() => {
    if (comment?.data?.length) {
      const mapping = comment?.data?.map((parent) => {
        const {
          id,
          create_by,
          createdAt,
  ...
  useEffect(() => {
    const response = listDocument?.data?.daftar_isi;
    if (response?.length) {
  ...
  useEffect(() => {
    if (workflowDetailEWP?.data) {
      ...
      newWorkflowData.ref_tim_audit_maker = `${maker?.pn} - ${maker?.name}`;
  ...
  const handleSubmitCommentParent = async (e, value) => {
    e.stopPropagation();
    const body = {
      ref_bab_kkpa_id: activeIndexComment.toString(),
      deskripsi: value,
    };

    await usePostData(
      `${process.env.NEXT_PUBLIC_API_URL_EWP}/ewp/sbp/kkpa/comment/${kkpa_id}?ref_bab_kkpa_id=${activeIndexComment}`,
      body
    );
  ...
  return (
    <LandingLayoutEWPConsulting>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <PrevNextNavigation
          baseUrl={pathNameSubModulAnalisa}
          routes={routes}
          prevUrl={"/rekomendasi"}
          marginLeft={"-45px"}
        />
