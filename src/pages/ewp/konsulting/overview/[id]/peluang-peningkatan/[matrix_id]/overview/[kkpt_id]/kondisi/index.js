...
import { useDetailKondisi } from "@/data/ewp/konsulting/peluang-peningkatan/matrix";
...
const index = () => {
  ...
  const { kondisiData, kondisiDataMutate } = useDetailKondisi({
    id: kkpt_id,
  });
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      {
        name: `Kondisi`,
        path: `${pathNameContent}/kondisi`,
      },
    ]);
  }, [projectDetail]);

  useEffect(() => {
    setContent(kondisiData?.data?.kondisi || "");
  }, [kondisiData]);
