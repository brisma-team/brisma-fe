import { Modal, Card, TextInput, Select } from "@/components/atoms";
import Button from "@atlaskit/button";
import { useEffect, useState } from "react";

const ModalSelectSourceData = ({ showModal, setShowModal, sourceType }) => {
  const [menu, setMenu] = useState(0);
  const [tag, setTag] = useState("P.A.T");
  const [source, setSource] = useState(1);
  const [arr, setArr] = useState(0);
  useEffect(() => {
    switch (sourceType) {
      case 1:
        setTag("P.A.T");
        break;
      case 2:
        setTag("E.W.P");
        break;
      case 3:
        setTag("R.P.M");
        break;
      default:
        setTag("P.A.T");
        break;
    }
  }, [sourceType]);

  const ar = [
    [
      "Data 2012",
      "Data 2013",
      "Data 2014",
      "Data 2015",
      "Data 2016",
      "Data 2017",
      "Data 2018",
      "Data 2019",
      "Data 2020",
      "Data 2021",
      "Data 2022",
    ],
    ["Data 2022", "Data 2023"],
  ];

  return (
    <Modal showModal={showModal} onClickOutside={() => setShowModal(false)}>
      {menu == 0 && (
        <div className="w-[50rem] h-modal p-2">
          <h3 className="p-3 font-bold text-lg">{`Pustaka Dokumen ` + tag}</h3>
          <div className="grid grid-cols-2">
            <div className="p-3">
              <Card>
                <div className="p-5 text-center">
                  <p className="font-bold text-lg">Brisma 1.0</p>
                  <h5 className="text-slate-400 mb-5 text-base font-semibold">
                    Pustaka Dokumen <br /> Tahun 2012 - 2022
                  </h5>
                  <Button
                    appearance="primary"
                    onClick={() => {
                      setSource(1);
                      setMenu(1);
                    }}
                  >
                    <p className="text-base p-1">Akses Data</p>
                  </Button>
                </div>
              </Card>
            </div>
            <div className="p-3">
              <Card>
                <div className="p-5 text-center">
                  <p className="font-bold text-lg">Brisma 2.0</p>
                  <p className="text-slate-400 mb-5 text-base font-semibold">
                    Pustaka Dokumen <br /> Tahun 2023 - Saat Ini
                  </p>
                  <Button
                    appearance="primary"
                    onClick={() => {
                      setSource(2);
                      setMenu(1);
                    }}
                  >
                    <p className="text-base p-1">Akses Data</p>
                  </Button>
                </div>
              </Card>
            </div>
          </div>
        </div>
      )}
      {menu == 1 && (
        <div className="w-[60rem] h-modal p-4">
          <h3 className="p-3 font-bold text-lg">Pustaka Data {tag}</h3>
          <div className="grid grid-cols-5">
            <div className="p-1 col-span-2">
              <Card>
                <h4 className="p-3 font-semibold text-lg">Pilih Tahun</h4>
                <div className="grid grid-cols-3 gap-2 p-4">
                  {ar[source - 1].map((item, i) => {
                    return (
                      <button
                        className={`p-2 text-base border border-slate-300 rounded-md shadow-sm hover:text-white hover:bg-gray-800 ${
                          arr == i ? "bg-gray-800 text-white" : ""
                        }`}
                        key={i}
                        onClick={() => setArr(i)}
                      >
                        {item}
                      </button>
                    );
                  })}
                </div>
              </Card>
            </div>
            <div className="p-1 col-span-3">
              <Card>
                <h4 className="p-3 font-semibold text-lg">Filter Data</h4>
                <div className="grid grid-cols-4">
                  <div className="p-3 font-semibold text-base">
                    Nama Project
                  </div>
                  <div className="p-1 pl-10 col-span-3">
                    <TextInput placeholder="Masukkan nama" />
                  </div>
                  <div className="p-3 font-semibold text-base">Tahun Audit</div>
                  <div className="p-1 pl-10 col-span-3">
                    <TextInput placeholder="Masukkan tahun" />
                  </div>
                  <div className="p-3 font-semibold text-base">Tipe Audit</div>
                  <div className="p-1 pl-10 col-span-3">
                    <Select
                      optionValue={[
                        { label: "Reguler", value: "Reguler" },
                        { label: "Special", value: "Special" },
                      ]}
                      placeholder="Pilih Tipe"
                      isSearchable={false}
                    />
                  </div>
                </div>
                <div className="flex justify-between gap-2 p-5">
                  <Button appearance="default" onClick={() => setMenu(0)}>
                    Kembali
                  </Button>
                  <Button appearance="primary" href="/catalogue/pat">
                    Tampilkan
                  </Button>
                </div>
              </Card>
            </div>
          </div>
        </div>
      )}
    </Modal>
  );
};

export default ModalSelectSourceData;
