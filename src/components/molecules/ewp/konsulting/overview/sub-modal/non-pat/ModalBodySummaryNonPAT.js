...
const ModalBodySummaryNonPAT = ({ data, setCurrentModalStage, isPat }) => {
  ...
  const findKTA = () => {
    const result = data.tim_audit?.kta?.find((v) => v.pn_kta === user.data.pn);
    if (result) {
        ...
        <FormWithLabel
          form={
            <TextInput
              ...
              value={
                isPat ? (data.tim_audit ? findKTA() : "") : user.data.fullName
              }
              isDisabled={true}
            />
          }
