...
const ModalBodyInfoKegiatan = ({
	...
	useEffect(() => {
		...
		if (
			...
			dispatch(
				setAuditTeamData([...mappedMA, ...mappedKTA, ...mappedATA])
			);
		}
	}, [auditTeam]);
				...
				<div className="w-full rounded-xl">
					<TextInput
						icon={
							<ButtonIcon
								handleClick={() =>
									handleChange("name_kegiatan_audit", "")
								}
				...
				<div className="flex gap-3 justify-between my-3">
					...
					<div className="w-1/2 h-fit">
						<CardMakeAuditTeam
							...
							anggota_tim_audit={
								auditTeam?.data?.ref_tim_audit_ata
							}
						/>
					</div>
				</div>
