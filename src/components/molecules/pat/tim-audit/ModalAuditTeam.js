...
const ModalAuditTeam = ({
	...
	useEffect(() => {
		if (typeModal === "detail") {
			...
			const mapppedData = {
				...
				ref_tim_audit_ma: auditTeam?.data?.ref_tim_audit_mas?.map(
					(v) => ({
						pn: v?.pn_ma,
						nama: v?.nama_ma,
						jabatan: v?.jabatan,
				...
				ref_tim_audit_kta: auditTeam?.data?.ref_tim_audit_kta?.map(
					(v) => ({
						pn: v?.pn_kta,
						nama: v?.nama_kta,
						jabatan: v?.jabatan,
				...
				ref_tim_audit_ata: auditTeam?.data?.ref_tim_audit_ata?.map(
					(v) => ({
						pn: v?.pn_ata,
						nama: v?.nama_ata,
						jabatan: v?.jabatan,
	...
	const handleAdd = (property) => {
		...
		dispatch(
			setAuditTeamData({ ...auditTeamData, [property]: newTimAudit })
		);
	};
	...
	const handleAddATA = () => {
		...
		dispatch(
			setAuditTeamData({
				...auditTeamData,
				ref_tim_audit_ata: newTimAuditATA,
			})
		);
	};
	...
	return (
		<Modal showModal={showModal}>
			<div className="w-[63rem]">
				<div className="w-[63rem] rounded-md flex items-center justify-center relative p-4 bg-white">
					<p className="w-full font-bold text-xl">Buat Tim Audit</p>
					<CloseModal
				...
				<form
					className="w-[63rem] rounded-md relative p-4 bg-white"
					onSubmit={handleSubmit}
				>
					<div className="p-4 border-gray-200 border rounded-md">
						<div className="w-full">
							<TextInput
								...
								className={
									"font-medium text-3xl rounded text-brisma"
								}
